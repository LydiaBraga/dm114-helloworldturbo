package br.inatel.lydia.helloworldturbo.webservice;
...
import br.inatel.lydia.helloworldturbo.models.AccessToken;
import br.inatel.lydia.helloworldturbo.utils.WSConstants;
import br.inatel.lydia.helloworldturbo.utils.WSUtil;
...
public class WebServiceClient {
    ...
    private static WebServiceResponse authenticate(Context context) {
        ...
        try {
            ...
            if (conn.getResponseCode() == 200) {
                ...
                long accessTokenExpiration = Calendar.getInstance().getTime().getTime() + accessToken.getExpires_in() * 1000;

                SharedPreferences.Editor editor = sharedSettings.edit();
                editor.putString(context.getString(R.string.pref_ws_access_token), accessToken.getAccess_token());
                editor.putLong(context.getString(R.string.pref_ws_access_token_expiration), accessTokenExpiration);
                editor.commit();
    ...
    private static String generateGrantType(String username, String password) {
        String grantType = "grant_type=password&username=" + username
                + "&password=" + password;
    ...
    private static boolean isTokenValid(Context context) {
        ...
        if (sharedSettings.contains(context.getString(R.string.pref_ws_access_token))
